...
class PostGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Post entity")
        ...
        .repeat(2) {
            ...
            .get("/backend/api/posts")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new post")
            .post("/backend/api/posts")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "title":"SAMPLE_TEXT", "details":"SAMPLE_TEXT"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_post_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created post")
                .get("/backend${new_post_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created post")
            .delete("/backend${new_post_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
